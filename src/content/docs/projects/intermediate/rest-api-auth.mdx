...
## Abstract
This REST API project demonstrates professional API development with Flask, featuring JWT (JSON Web Token) authentication, user management, CRUD operations, input validation, rate limiting, and comprehensive documentation. The API includes user registration, login, protected endpoints, role-based access control, and admin functionalities for managing users and resources.
...
pip install flask flask-sqlalchemy flask-jwt-extended flask-limiter flask-cors marshmallow flask-migrate python-dotenv
```

## Getting Started
### Setting Up the Project
1. Create a folder named `rest-api-auth`
2. Create the main application file `restapi.py`
3. Create configuration files and environment variables
