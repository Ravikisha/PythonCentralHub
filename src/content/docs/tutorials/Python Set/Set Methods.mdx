...
## Try it: Set Methods Exercises

### Exercise 1 – issubset() and issuperset()

<DataCampExercise
  lang="python"
  hint={`Use .issubset() and .issuperset() to compare sets.`}
  code={`# Task: Exercise 1 – issubset() and issuperset()
...
# Step: Assign the correct value (a dict or set) to 'a'.
a = ___  # replace ___ with the correct value
# Hint: Use .issubset() and .issuperset() to compare sets.
# Step: Assign the correct value (a dict or set) to 'b'.
...
success_msg("issubset() and issuperset() work!")`}
  height={150}
/>
...
<DataCampExercise
  lang="python"
  hint={`Use .copy() so modifying the copy doesn't affect the original.`}
  code={`# Task: Exercise 2 – copy()
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!

# Hint: print(value) displays output; print(a, b) prints multiple values.
# Step: Assign the correct value (a dict or set) to 's'.
s = ___  # replace ___ with the correct value
# Hint: Use .copy() so modifying the copy doesn't affect the original.
# Step: Assign the correct value to 's2'.
s2 = ___  # replace ___ with the correct value
# Hint: Run after every change -- small steps are easier to debug!
...
# Hint: Read error messages carefully -- they point to the exact line.
# Step: Call print() with the right argument(s).
# Example structure: print("Original size:", len(s))
print(___)  # replace ___ with the correct expression
# Step: Call print() with the right argument(s).
# Example structure: print("Copy size:", len(s2))
print(___)  # replace ___ with the correct expression
...
print("Copy size:", len(s2))`}
  sct={`test_output_contains("Original size: 3")
test_output_contains("Copy size: 4")
success_msg("Set copy works!")`}
  height={160}
/>

### Exercise 3 – isdisjoint()

<DataCampExercise
  lang="python"
  hint={`isdisjoint() returns True if no elements are shared.`}
  code={`# Task: Exercise 3 – isdisjoint()
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!
...
b = ___  # replace ___ with the correct value
# Hint: Run after every change -- small steps are easier to debug!
# Step: Assign the correct value (a dict or set) to 'c'.
c = ___  # replace ___ with the correct value
# Hint: Read error messages carefully -- they point to the exact line.
# Step: Call print() with the right argument(s).
# Example structure: print(a.isdisjoint(b))
print(___)  # replace ___ with the correct expression
# Step: Call print() with the right argument(s).
# Example structure: print(a.isdisjoint(c))
print(___)  # replace ___ with the correct expression
