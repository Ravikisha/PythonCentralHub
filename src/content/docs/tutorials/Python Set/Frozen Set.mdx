...
## Try it: Frozen Set Exercises

### Exercise 1 – Create a Frozen Set

<DataCampExercise
  lang="python"
  hint={`Use frozenset() to create an immutable set.`}
  code={`# Task: Exercise 1 – Create a Frozen Set
...
# Step: Assign the correct value to 'fs'.
fs = ___  # replace ___ with the correct value
# Hint: Use frozenset() to create an immutable set.
# Step: Call print() with the right argument(s).
# Example structure: print(type(fs).__name__)
print(___)  # replace ___ with the correct expression

# ── Expected Output ──────────────────────────────────────────
# (Run your completed code to check the output)
# ──────────────────────────────────────────────────────────────`}
  solution={`fs = frozenset([1, 2, 3, 4])
print(type(fs).__name__)`}
  sct={`test_output_contains("frozenset")
success_msg("frozenset creation works!")`}
  height={120}
/>

### Exercise 2 – Frozen Set Operations

<DataCampExercise
  lang="python"
  hint={`Frozen sets support union, intersection, and difference.`}
  code={`# Task: Exercise 2 – Frozen Set Operations
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!

# Hint: print(value) displays output; print(a, b) prints multiple values.
# Step: Assign the correct value to 'fs1'.
fs1 = ___  # replace ___ with the correct value
# Hint: Frozen sets support union, intersection, and difference.
# Step: Assign the correct value to 'fs2'.
fs2 = ___  # replace ___ with the correct value
# Hint: Run after every change -- small steps are easier to debug!
# Step: Call print() with the right argument(s).
# Example structure: print(len(fs1 | fs2))
print(___)  # replace ___ with the correct expression
...
success_msg("frozenset operations work!")`}
  height={140}
/>

### Exercise 3 – Use as Dict Key

<DataCampExercise
  lang="python"
  hint={`Frozen sets are hashable so they can be dictionary keys.`}
  code={`# Task: Exercise 3 – Use as Dict Key
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!
...
d = {fs: "my key"}
print(d[fs])`}
  sct={`test_output_contains("my key")
success_msg("frozenset as dict key works!")`}
  height={130}
/>
