...
## Try it: File Handling Exercises

### Exercise 1 – Write to a File

<DataCampExercise
  lang="python"
  hint={`Use open() with mode "w" to write, then close the file.`}
  code={`# Task: Exercise 1 – Write to a File
...
# Example structure: print("File written")
print(___)  # replace ___ with the correct expression

# ── Expected Output ──────────────────────────────────────────
# File written
# ──────────────────────────────────────────────────────────────`}
  solution={`with open("/tmp/test.txt", "w") as f:
    f.write("Hello, Python!")
print("File written")`}
  sct={`test_output_contains("File written")
success_msg("File writing works!")`}
  height={140}
/>
...
<DataCampExercise
  lang="python"
  hint={`Use open() with mode "r" to read. Use with to auto-close.`}
  code={`# Task: Exercise 2 – Read a File
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!
...
# replace the comment above with working code
    ...
    # Step: Assign the correct value to 'content'.
    content = ___  # replace ___ with the correct value
# Step: Call print() with the right argument(s).
# Example structure: print(content)
print(___)  # replace ___ with the correct expression
...
success_msg("File reading works!")`}
  height={180}
/>
...
<DataCampExercise
  lang="python"
  hint={`Use mode "a" to append without overwriting existing content.`}
  code={`# Task: Exercise 3 – Append to a File
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!
...
# (Run your completed code to check the output)
# ──────────────────────────────────────────────────────────────`}
  solution={`with open("/tmp/log.txt", "w") as f:
    f.write("Line 1\n")
with open("/tmp/log.txt", "a") as f:
    f.write("Line 2\n")
with open("/tmp/log.txt", "r") as f:
...
success_msg("File appending works!")`}
  height={200}
/>
