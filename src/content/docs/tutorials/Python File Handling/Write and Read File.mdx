...
### Exercise 1 – Write and Read Text

<DataCampExercise
  lang="python"
  hint={`Write a string to a file then read it back.`}
  code={`# Task: Exercise 1 – Write and Read Text
...
# ──────────────────────────────────────────────────────────────`}
  solution={`with open("/tmp/note.txt", "w") as f:
    f.write("Python file handling is easy!")
with open("/tmp/note.txt", "r") as f:
    print(f.read())`}
  sct={`test_output_contains("Python file handling is easy!")
success_msg("Write and read works!")`}
  height={170}
/>

### Exercise 2 – Loop and Read Lines

<DataCampExercise
  lang="python"
  hint={`Use a for loop to iterate over lines in an open file.`}
  code={`# Task: Exercise 2 – Loop and Read Lines
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!
...
# replace the comment above with working code
    ...
    for line in f:
        # Hint: Read error messages carefully -- they point to the exact line.
        # Step: Call print() with the right argument(s).
        # Example structure: print(line.strip())
        print(___)  # replace ___ with the correct expression
...
success_msg("Line-by-line reading works!")`}
  height={190}
/>

### Exercise 3 – Count Words in File

<DataCampExercise
  lang="python"
  hint={`Read the file content and use split() to count words.`}
  code={`# Task: Exercise 3 – Count Words in File
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!
...
# (Run your completed code to check the output)
# ──────────────────────────────────────────────────────────────`}
  solution={`with open("/tmp/words.txt", "w") as f:
    f.write("Python is a great language")
with open("/tmp/words.txt", "r") as f:
    words = f.read().split()
print("Word count:", len(words))`}
  sct={`test_output_contains("Word count: 5")
success_msg("Word counting from file works!")`}
  height={180}
/>
