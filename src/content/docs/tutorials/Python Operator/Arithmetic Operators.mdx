...
### Exercise 1 – Division Types

<DataCampExercise
  lang="python"
  hint={`/ gives float division, // gives integer (floor) division.`}
  code={`# Task: Exercise 1 – Division Types
...
print(10 // 3)
print(10 % 3)`}
  sct={`test_output_contains("3.3333")
test_output_contains("3")
success_msg("Division types work!")`}
  height={140}
/>

### Exercise 2 – Power Operator

<DataCampExercise
  lang="python"
  hint={`Use ** to raise a number to a power.`}
  code={`# Task: Exercise 2 – Power Operator
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!

# Hint: print(value) displays output; print(a, b) prints multiple values.
# Step: Call print() with the right argument(s).
# Example structure: print(2 ** 8)
print(___)  # replace ___ with the correct expression
# Hint: Use ** to raise a number to a power.
# Step: Call print() with the right argument(s).
# Example structure: print(9 ** 0.5)
print(___)  # replace ___ with the correct expression
...
# ──────────────────────────────────────────────────────────────`}
  solution={`print(2 ** 8)
print(9 ** 0.5)`}
  sct={`test_output_contains("256")
test_output_contains("3.0")
success_msg("Power operator works!")`}
  height={130}
/>

### Exercise 3 – Compound Expression

<DataCampExercise
  lang="python"
  hint={`Combine multiple arithmetic operators in one expression.`}
  code={`# Task: Exercise 3 – Compound Expression
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!

# Hint: print(value) displays output; print(a, b) prints multiple values.
# Step: Assign the correct value (a tuple or function call) to 'result'.
...
# (Run your completed code to check the output)
# ──────────────────────────────────────────────────────────────`}
  solution={`result = (10 + 5) * 2 - 8 // 3
print(result)`}
  sct={`test_output_contains("28")
success_msg("Compound arithmetic expression works!")`}
  height={120}
/>
