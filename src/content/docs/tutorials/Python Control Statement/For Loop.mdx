...
## Try it: For Loop Exercises

### Exercise 1 – Loop over a List

<DataCampExercise
  lang="python"
  hint={`Use a for loop to iterate over the fruits list and print each item.`}
  code={`# Task: Exercise 1 – Loop over a List
...
success_msg("Great! You looped over a list!")`}
  height={160}
/>

### Exercise 2 – Using range()

<DataCampExercise
  lang="python"
  hint={`Use range(1, 6) to print numbers from 1 to 5.`}
  code={`# Task: Exercise 2 – Using range()
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!
...
success_msg("range() works perfectly!")`}
  height={130}
/>
...
<DataCampExercise
  lang="python"
  hint={`Use two for loops to create a multiplication table for 2 and 3.`}
  code={`# Task: Exercise 3 – Nested For Loop
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!
...
for i in [2, 3]:
    ...
    for j in range(1, 4):
        # Hint: Use two for loops to create a multiplication table for 2 and 3.
        # Step: Call print() with the right argument(s).
        # Example structure: print(i, "x", j, "=", i * j)
        print(___)  # replace ___ with the correct expression

# ── Expected Output ──────────────────────────────────────────
# (Run your completed code to check the output)
# ──────────────────────────────────────────────────────────────`}
  solution={`for i in [2, 3]:
    for j in range(1, 4):
        print(i, "x", j, "=", i * j)`}
  sct={`test_output_contains("2 x 1 = 2")
test_output_contains("3 x 3 = 9")
success_msg("Nested loops mastered!")`}
  height={160}
/>
