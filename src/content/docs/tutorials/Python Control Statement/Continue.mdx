...
## Try it: Continue Statement Exercises

### Exercise 1 – Skip Even Numbers

<DataCampExercise
  lang="python"
  hint={`Use continue to skip even numbers and only print odd numbers.`}
  code={`# Task: Exercise 1 – Skip Even Numbers
...
success_msg("continue skips even numbers correctly!")`}
  height={160}
/>

### Exercise 2 – Skip a Specific Value

<DataCampExercise
  lang="python"
  hint={`Use continue to skip "banana" but print all other fruits.`}
  code={`# Task: Exercise 2 – Skip a Specific Value
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!
...
for fruit in fruits:
    ...
    # Step: Call print() with the right argument(s).
    # Example structure: print(fruit)
    print(___)  # replace ___ with the correct expression

# ── Expected Output ──────────────────────────────────────────
# (Run your completed code to check the output)
# ──────────────────────────────────────────────────────────────`}
  solution={`fruits = ["apple", "banana", "cherry", "mango"]
for fruit in fruits:
    if fruit == "banana":
        continue
    print(fruit)`}
  sct={`test_output_contains("apple")
test_output_contains("cherry")
success_msg("continue skips the right value!")`}
  height={170}
/>

### Exercise 3 – Continue in While Loop

<DataCampExercise
  lang="python"
  hint={`Use continue to skip multiples of 3 in the while loop.`}
  code={`# Task: Exercise 3 – Continue in While Loop
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!
...
while n < 10:
    ...
    print(n)`}
  sct={`test_output_contains("1")
test_output_contains("10")
success_msg("continue in while loop works!")`}
  height={180}
/>
