...
<DataCampExercise
  lang="python"
  hint={`Use .update() to add or overwrite key-value pairs.`}
  code={`# Task: Exercise 1 – update()
...
# Step: Assign the correct value (a dict or set) to 'person'.
person = ___  # replace ___ with the correct value
# Hint: Use .update() to add or overwrite key-value pairs.
# Step: Write the required code here.
# person.update({"age": 26, "city": "NYC"})
...
test_output_contains("NYC")
success_msg("update() method works!")`}
  height={160}
/>
...
<DataCampExercise
  lang="python"
  hint={`Use pop() to remove a specific key and return its value.`}
  code={`# Task: Exercise 2 – pop()
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!
...
# Hint: Use pop() to remove a specific key and return its value.
# Step: Assign the correct value to 'removed'.
removed = ___  # replace ___ with the correct value
# Hint: Run after every change -- small steps are easier to debug!
# Step: Call print() with the right argument(s).
# Example structure: print("Removed:", removed)
print(___)  # replace ___ with the correct expression
# Hint: Read error messages carefully -- they point to the exact line.
# Step: Call print() with the right argument(s).
# Example structure: print("Remaining:", d)
print(___)  # replace ___ with the correct expression
...
print("Remaining:", d)`}
  sct={`test_output_contains("Removed: 2")
success_msg("pop() method works!")`}
  height={150}
/>

### Exercise 3 – values() Sum

<DataCampExercise
  lang="python"
  hint={`Use .values() and sum() to calculate total price.`}
  code={`# Task: Exercise 3 – values() Sum
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!
...
# Hint: Use .values() and sum() to calculate total price.
# Step: Call print() with the right argument(s).
# Example structure: print("Total:", sum(prices.values()))
print(___)  # replace ___ with the correct expression

# ── Expected Output ──────────────────────────────────────────
# (Run your completed code to check the output)
# ──────────────────────────────────────────────────────────────`}
  solution={`prices = {"apple": 1.5, "banana": 0.8, "cherry": 2.0}
print("Total:", sum(prices.values()))`}
  sct={`test_output_contains("Total: 4.3")
success_msg("values() and sum work!")`}
  height={130}
/>
