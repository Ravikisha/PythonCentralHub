...
## Try it: Dictionary Operations Exercises

### Exercise 1 – Merge Dictionaries

<DataCampExercise
  lang="python"
  hint={`Use ** unpacking to merge two dicts into one.`}
  code={`# Task: Exercise 1 – Merge Dictionaries
...
# Step: Assign the correct value (a dict or set) to 'd1'.
d1 = ___  # replace ___ with the correct value
# Hint: Use ** unpacking to merge two dicts into one.
# Step: Assign the correct value (a dict or set) to 'd2'.
d2 = ___  # replace ___ with the correct value
# Hint: Run after every change -- small steps are easier to debug!
# Step: Assign the correct value (a dict or set) to 'merged'.
merged = ___  # replace ___ with the correct value
# Hint: Read error messages carefully -- they point to the exact line.
# Step: Call print() with the right argument(s).
# Example structure: print(merged)
...
test_output_contains("d")
success_msg("Dictionary merge works!")`}
  height={140}
/>
...
<DataCampExercise
  lang="python"
  hint={`Use dict comprehension to double all values.`}
  code={`# Task: Exercise 2 – Dictionary Comprehension
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!

# Hint: Dict comprehension: {key: value for item in iterable}
# Step: Assign the correct value (a dict or set) to 'prices'.
prices = ___  # replace ___ with the correct value
# Hint: print(value) displays output; print(a, b) prints multiple values.
# Step: Assign the correct value (a dict or set) to 'doubled'.
doubled = ___  # replace ___ with the correct value
# Hint: Use dict comprehension to double all values.
# Step: Call print() with the right argument(s).
# Example structure: print(doubled)
print(___)  # replace ___ with the correct expression
...
success_msg("Dictionary comprehension works!")`}
  height={140}
/>
...
<DataCampExercise
  lang="python"
  hint={`Use a dict and .get() to count each word's frequency.`}
  code={`# Task: Exercise 3 – Word Frequency Counter
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!

# Hint: 'for item in iterable:' iterates over each element in sequence.
# Step: Assign the correct value (a list) to 'words'.
words = ___  # replace ___ with the correct value
# Hint: print(value) displays output; print(a, b) prints multiple values.
# Step: Assign the correct value (a dict or set) to 'freq'.
freq = ___  # replace ___ with the correct value
# Hint: Use a dict and .get() to count each word's frequency.
# Step: Write a for loop -- 'for <variable> in <iterable>:'
for word in words:
    # Hint: Run after every change -- small steps are easier to debug!
    # Step: Assign the correct value to 'freq[word]'.
    freq[word] = ___  # replace ___ with the correct value
# Hint: Read error messages carefully -- they point to the exact line.
# Step: Call print() with the right argument(s).
# Example structure: print(freq["apple"])
print(___)  # replace ___ with the correct expression

# ── Expected Output ──────────────────────────────────────────
# (Run your completed code to check the output)
# ──────────────────────────────────────────────────────────────`}
  solution={`words = ["apple", "banana", "apple", "cherry", "apple"]
freq = {}
for word in words:
    freq[word] = freq.get(word, 0) + 1
print(freq["apple"])`}
  sct={`test_output_contains("3")
success_msg("Word frequency counting works!")`}
  height={170}
/>
