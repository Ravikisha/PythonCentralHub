...
# Hint: Assign a value to a new key to add it to the dictionary.
# Step: Assign the correct value (a number) to 'd["c"]'.
d["c"] = ___  # replace ___ with the correct value
# Hint: Run after every change -- small steps are easier to debug!
# Step: Call print() with the right argument(s).
# Example structure: print(d)
...
success_msg("Adding a key works!")`}
  height={120}
/>

### Exercise 2 – Remove with del

<DataCampExercise
  lang="python"
  hint={`Use del to remove a key from the dictionary.`}
  code={`# Task: Exercise 2 – Remove with del
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!
...
del d["y"]
print(d)`}
  sct={`test_output_contains("x")
success_msg("del removes key correctly!")`}
  height={120}
/>
...
<DataCampExercise
  lang="python"
  hint={`Use .clear() to remove all items from the dictionary.`}
  code={`# Task: Exercise 3 – clear()
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!
...
d.clear()
print(d)`}
  sct={`test_output_contains("{}")
success_msg("clear() empties the dictionary!")`}
  height={120}
/>
