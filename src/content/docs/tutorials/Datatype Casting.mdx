...
## Try it: Type Casting Exercises

### Exercise 1 – String to Number

<DataCampExercise
  lang="python"
  hint={`Use int("42") to convert a string "42" to the integer 42.`}
  code={`# Task: Exercise 1 – String to Number
...
# Step: Assign the correct value (a string) to 's_int'.
s_int = ___  # replace ___ with the correct value
# Hint: Use int("42") to convert a string "42" to the integer 42.
# Step: Assign the correct value (a string) to 's_float'.
s_float = ___  # replace ___ with the correct value

# Hint: Run after every change -- small steps are easier to debug!
# Step: Assign the correct value to 'n'.
n = ___  # replace ___ with the correct value
# Hint: Read error messages carefully -- they point to the exact line.
# Step: Assign the correct value to 'f'.
f = ___  # replace ___ with the correct value
...
print(___)  # replace ___ with the correct expression
# Step: Call print() with the right argument(s).
# Example structure: print(type(n), type(f))
print(___)  # replace ___ with the correct expression
...
print(f * 2)
print(type(n), type(f))`}
  sct={`test_output_contains("50")
test_output_contains("6.28")
test_output_contains("int")
test_output_contains("float")
success_msg("String to number conversion works!")`}
  height={220}
/>
...
<DataCampExercise
  lang="python"
  hint={`Use str() to convert numbers to strings, then concatenate with +.`}
  code={`# Task: Exercise 2 – Number to String
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!
...
# Hint: Use str() to convert numbers to strings, then concatenate with +.
# Step: Assign the correct value (a number) to 'height'.
height = ___  # replace ___ with the correct value

# Hint: Run after every change -- small steps are easier to debug!
# Step: Assign the correct value (a string) to 'message'.
message = ___  # replace ___ with the correct value
# Hint: Read error messages carefully -- they point to the exact line.
# Step: Call print() with the right argument(s).
# Example structure: print(message)
print(___)  # replace ___ with the correct expression
...
print(message)`}
  sct={`test_output_contains("Age: 25, Height: 1.75")
success_msg("Number to string conversion works!")`}
  height={160}
/>

### Exercise 3 – List, Tuple, and Set Conversions

<DataCampExercise
  lang="python"
  hint={`Use list(), tuple(), set() to convert between collection types.`}
  code={`# Task: Exercise 3 – List, Tuple, and Set Conversions
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!

# Hint: print(value) displays output; print(a, b) prints multiple values.
# Step: Assign the correct value (a list) to 'my_list'.
my_list = ___  # replace ___ with the correct value

# Hint: Use list(), tuple(), set() to convert between collection types.
# Step: Assign the correct value to 'as_tuple'.
as_tuple = ___  # replace ___ with the correct value
# Hint: Run after every change -- small steps are easier to debug!
# Step: Assign the correct value to 'as_set'.
as_set = ___  # replace ___ with the correct value

# Hint: Read error messages carefully -- they point to the exact line.
# Step: Call print() with the right argument(s).
# Example structure: print(as_tuple)
print(___)  # replace ___ with the correct expression
# Step: Call print() with the right argument(s).
# Example structure: print(as_set)
print(___)  # replace ___ with the correct expression
# Step: Call print() with the right argument(s).
# Example structure: print(type(as_tuple), type(as_set))
print(___)  # replace ___ with the correct expression

# ── Expected Output ──────────────────────────────────────────
# (Run your completed code to check the output)
# ──────────────────────────────────────────────────────────────`}
  solution={`my_list = [1, 2, 3, 2, 1]
as_tuple = tuple(my_list)
as_set = set(my_list)
print(as_tuple)
print(as_set)
print(type(as_tuple), type(as_set))`}
  sct={`test_output_contains("(1, 2, 3, 2, 1)")
test_output_contains("tuple")
test_output_contains("set")
success_msg("Collection type conversions mastered!")`}
  height={200}
/>