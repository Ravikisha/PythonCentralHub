...
## Try it: Lambda Function Exercises

### Exercise 1 – Basic Lambda

<DataCampExercise
  lang="python"
  hint={`A lambda is an anonymous one-liner function: lambda args: expression`}
  code={`# Task: Exercise 1 – Basic Lambda
...
success_msg("Lambda function works!")`}
  height={120}
/>
...
<DataCampExercise
  lang="python"
  hint={`Use a lambda as the key to sort by the second element.`}
  code={`# Task: Exercise 2 – Lambda with sorted()
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!

# Hint: Lambda syntax: lambda args: expression  (single-line function)
# Step: Assign the correct value (a list) to 'pairs'.
pairs = ___  # replace ___ with the correct value
# Hint: print(value) displays output; print(a, b) prints multiple values.
...
# Hint: Use a lambda as the key to sort by the second element.
# Step: Call print() with the right argument(s).
# Example structure: print(pairs)
print(___)  # replace ___ with the correct expression

# ── Expected Output ──────────────────────────────────────────
# (Run your completed code to check the output)
# ──────────────────────────────────────────────────────────────`}
  solution={`pairs = [(1, 3), (2, 1), (3, 2)]
pairs.sort(key=lambda x: x[1])
print(pairs)`}
  sct={`test_output_contains("(2, 1)")
success_msg("Lambda with sorted works!")`}
  height={140}
/>

### Exercise 3 – Lambda with map()

<DataCampExercise
  lang="python"
  hint={`Use map() with a lambda to apply a function to every element.`}
  code={`# Task: Exercise 3 – Lambda with map()
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!
...
# Hint: print(value) displays output; print(a, b) prints multiple values.
# Step: Assign the correct value to 'doubled'.
...
print(doubled)`}
  sct={`test_output_contains("[2, 4, 6, 8, 10]")
success_msg("Lambda with map() works!")`}
  height={140}
/>
