---
title: Multiple Assignment
description: How to assign multiple values to multiple variables in Python. Learn about multiple assignment, one value to multiple variables, multiple values to one variable, and unpacking a tuple or list.
sidebar: 
    order: 8
---

## Many Values to Multiple Variables
We can assign multiple values to multiple variables in one line. This is called multiple assignment.

#### Example:
```python title="variable.py" showLineNumbers{1} {2,3,4}
# Multiple assignment
a, b, c = 10, 20.5, "Hello, World!"
print(a, b, c)
```

Output

```cmd title="command" showLineNumbers{1} {2-4}
C:\Users\Your Name> python variable.py
10 20.5 Hello, World!
```

:::caution
The number of variables on the left and the number of values on the right must be the same.
:::

## One Value to Multiple Variables
We can assign one value to multiple variables in one line. This is called one value to multiple variables.

#### Example:
```python title="variable.py" showLineNumbers{1} {2}
# One value to multiple variables
a = b = c = 10
print(a, b, c)
```

Output

```cmd title="command" showLineNumbers{1} {2-4}
C:\Users\Your Name> python variable.py
10 10 10
```

## Multiple Values to One Variable
We can assign multiple values to one variable in one line. This is called multiple values to one variable.

#### Example:
```python title="variable.py" showLineNumbers{1} {2}
# Multiple values to one variable
a = 10, 20, 30
print(a)
```

Output

```cmd title="command" showLineNumbers{1} {2-4}
C:\Users\Your Name> python variable.py
(10, 20, 30)
```

In this case, the variable `a` is a tuple.

## Unpacking a Tuple
We can unpack a tuple in multiple variables. This is called unpacking a tuple.

#### Example:
```python title="variable.py" showLineNumbers{1} {3}
# Unpacking a tuple
a = 10, 20, 30
x, y, z = a
print(x, y, z)
```

Output

```cmd title="command" showLineNumbers{1} {2-4}
C:\Users\Your Name> python variable.py
10 20 30
```

## Unpacking a List
We can unpack a list in multiple variables. This is called unpacking a list.

#### Example:
```python title="variable.py" showLineNumbers{1} {3}
# Unpacking a list
a = [10, 20, 30]
x, y, z = a
print(x, y, z)
```

Output

```cmd title="command" showLineNumbers{1} {2-4}
C:\Users\Your Name> python variable.py
10 20 30
```

---

import DataCampExercise from "../../../../components/DataCampExercise.astro";

## Try it: Multiple Assignment Exercises

### Exercise 1 – Assign Same Value to Multiple Variables

<DataCampExercise
  lang="python"
  hint={`Use chained assignment: a = b = c = 0`}
  code={`# Task: Exercise 1 – Assign Same Value to Multiple Variables
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!

# Hint: print(value) displays output; print(a, b) prints multiple values.
# Step: Assign the correct value to 'a'.
a = ___  # replace ___ with the correct value
# Hint: Use chained assignment: a = b = c = 0
# Step: Call print() with the right argument(s).
# Example structure: print(a, b, c)
print(___)  # replace ___ with the correct expression

# ── Expected Output ──────────────────────────────────────────
# (Run your completed code to check the output)
# ──────────────────────────────────────────────────────────────`}
  solution={`a = b = c = 0
print(a, b, c)`}
  sct={`test_output_contains("0 0 0")
success_msg("Multiple assignment with same value works!")`}
  height={120}
/>

### Exercise 2 – Tuple Unpacking

<DataCampExercise
  lang="python"
  hint={`Unpack the tuple (1, 2, 3) into variables x, y, z.`}
  code={`# Task: Exercise 2 – Tuple Unpacking
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!

# Hint: print(value) displays output; print(a, b) prints multiple values.
# Step: Write the required code here.
# x, y, z = (1, 2, 3)
# replace the comment above with working code
# Hint: Unpack the tuple (1, 2, 3) into variables x, y, z.
# Step: Call print() with the right argument(s).
# Example structure: print(x, y, z)
print(___)  # replace ___ with the correct expression

# ── Expected Output ──────────────────────────────────────────
# (Run your completed code to check the output)
# ──────────────────────────────────────────────────────────────`}
  solution={`x, y, z = (1, 2, 3)
print(x, y, z)`}
  sct={`test_output_contains("1 2 3")
success_msg("Tuple unpacking works!")`}
  height={120}
/>

### Exercise 3 – Assign from a List

<DataCampExercise
  lang="python"
  hint={`Unpack [10, 20, 30] into first, second, third.`}
  code={`# Task: Exercise 3 – Assign from a List
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!

# Hint: print(value) displays output; print(a, b) prints multiple values.
# Step: Write the required code here.
# first, second, third = [10, 20, 30]
# replace the comment above with working code
# Hint: Unpack [10, 20, 30] into first, second, third.
# Step: Call print() with the right argument(s).
# Example structure: print(first, second, third)
print(___)  # replace ___ with the correct expression

# ── Expected Output ──────────────────────────────────────────
# (Run your completed code to check the output)
# ──────────────────────────────────────────────────────────────`}
  solution={`first, second, third = [10, 20, 30]
print(first, second, third)`}
  sct={`test_output_contains("10 20 30")
success_msg("List unpacking works!")`}
  height={120}
/>

