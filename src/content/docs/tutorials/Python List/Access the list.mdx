...
<DataCampExercise
  lang="python"
  hint={`Access the second element using index 1.`}
...
# Step: Assign the correct value (a list) to 'colors'.
...
success_msg("Positive indexing works!")`}
  height={120}
/>
...
<DataCampExercise
  lang="python"
  hint={`Use -1 for the last element, -2 for second-to-last.`}
...
# Step: Call print() with the right argument(s).
# Example structure: print(colors[-2])
print(___)  # replace ___ with the correct expression
...
print(colors[-1])
print(colors[-2])`}
  sct={`test_output_contains("yellow")
test_output_contains("blue")
success_msg("Negative indexing works!")`}
  height={140}
/>
...
<DataCampExercise
  lang="python"
  hint={`Use [1:3] to get elements at index 1 and 2.`}
  code={`# Task: Exercise 3 – Slicing a List
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!
...
# Hint: Use [1:3] to get elements at index 1 and 2.
# Step: Call print() with the right argument(s).
# Example structure: print(nums[1:4])
print(___)  # replace ___ with the correct expression

# ── Expected Output ──────────────────────────────────────────
# (Run your completed code to check the output)
# ──────────────────────────────────────────────────────────────`}
  solution={`nums = [0, 10, 20, 30, 40, 50]
print(nums[1:4])`}
  sct={`test_output_contains("10")
test_output_contains("30")
success_msg("List slicing works!")`}
  height={130}
/>
