...
## Try it: List Methods Exercises

### Exercise 1 – append() and remove()

<DataCampExercise
  lang="python"
  hint={`Use append() to add "mango" then remove() to remove "banana".`}
  code={`# Task: Exercise 1 – append() and remove()
...
# Example structure: print(fruits)
...
fruits.remove("banana")
print(fruits)`}
  sct={`test_output_contains("mango")
success_msg("append() and remove() work!")`}
  height={150}
/>

### Exercise 2 – sort() and reverse()

<DataCampExercise
  lang="python"
  hint={`Sort the list then reverse it.`}
  code={`# Task: Exercise 2 – sort() and reverse()
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!

# Hint: print(value) displays output; print(a, b) prints multiple values.
# Step: Assign the correct value (a list) to 'nums'.
...
# Hint: Run after every change -- small steps are easier to debug!
# Step: Call print() with the right argument(s).
# Example structure: print("Sorted:", nums)
print(___)  # replace ___ with the correct expression
# Hint: Read error messages carefully -- they point to the exact line.
# Step: Write the required code here.
# nums.reverse()
# replace the comment above with working code
# Step: Call print() with the right argument(s).
# Example structure: print("Reversed:", nums)
print(___)  # replace ___ with the correct expression
...
nums.reverse()
print("Reversed:", nums)`}
  sct={`test_output_contains("Sorted:")
success_msg("sort() and reverse() work!")`}
  height={160}
/>

### Exercise 3 – index() and count()

<DataCampExercise
  lang="python"
  hint={`Use index() to find where "cherry" is and count() to count "apple".`}
  code={`# Task: Exercise 3 – index() and count()
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!
...
# Hint: Use index() to find where "cherry" is and count() to count "apple".
# Step: Call print() with the right argument(s).
# Example structure: print("cherry at:", fruits.index("cherry"))
print(___)  # replace ___ with the correct expression
# Hint: Run after every change -- small steps are easier to debug!
# Step: Call print() with the right argument(s).
# Example structure: print("apple count:", fruits.count("apple"))
print(___)  # replace ___ with the correct expression

# ── Expected Output ──────────────────────────────────────────
# (Run your completed code to check the output)
# ──────────────────────────────────────────────────────────────`}
  solution={`fruits = ["apple", "banana", "cherry", "apple"]
print("cherry at:", fruits.index("cherry"))
print("apple count:", fruits.count("apple"))`}
  sct={`test_output_contains("cherry at: 2")
test_output_contains("apple count: 2")
success_msg("index() and count() work!")`}
  height={160}
/>
