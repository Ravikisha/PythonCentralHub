...
## Try it: String Concatenation Exercises

### Exercise 1 – Concatenate with +

<DataCampExercise
  lang="python"
  hint={`Use + to join first_name and last_name with a space between them.`}
  code={`# Task: Exercise 1 – Concatenate with +
...
# Step: Assign the correct value (a string) to 'first_name'.
first_name = ___  # replace ___ with the correct value
# Hint: Use + to join first_name and last_name with a space between them.
# Step: Assign the correct value (a string) to 'last_name'.
last_name = ___  # replace ___ with the correct value
# Hint: Run after every change -- small steps are easier to debug!
# Step: Assign the correct value to 'full_name'.
full_name = ___  # replace ___ with the correct value
# Hint: Read error messages carefully -- they point to the exact line.
# Step: Call print() with the right argument(s).
# Example structure: print(full_name)
...
print(full_name)`}
  sct={`test_output_contains("John Doe")
success_msg("String concatenation with + works!")`}
  height={150}
/>

### Exercise 2 – Join a List of Strings

<DataCampExercise
  lang="python"
  hint={`Use ", ".join() to combine a list of words into one string.`}
  code={`# Task: Exercise 2 – Join a List of Strings
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!
...
# Hint: Run after every change -- small steps are easier to debug!
# Step: Call print() with the right argument(s).
# Example structure: print(sentence)
print(___)  # replace ___ with the correct expression
...
sentence = " ".join(words)
print(sentence)`}
  sct={`test_output_contains("Python is awesome")
success_msg("join() concatenation works!")`}
  height={140}
/>

### Exercise 3 – Repeat a String

<DataCampExercise
  lang="python"
  hint={`Use * to repeat the string "ha" three times.`}
  code={`# Task: Exercise 3 – Repeat a String
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!

# Hint: print(value) displays output; print(a, b) prints multiple values.
# Step: Assign the correct value (a string) to 'laugh'.
laugh = ___  # replace ___ with the correct value
# Hint: Use * to repeat the string "ha" three times.
# Step: Call print() with the right argument(s).
# Example structure: print(laugh)
print(___)  # replace ___ with the correct expression
...
# ──────────────────────────────────────────────────────────────`}
  solution={`laugh = "ha" * 3
print(laugh)`}
  sct={`test_output_contains("hahaha")
success_msg("String repetition with * works!")`}
  height={110}
/>
