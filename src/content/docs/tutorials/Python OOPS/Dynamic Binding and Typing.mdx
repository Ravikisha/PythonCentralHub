...
## Try it: Dynamic Binding & Typing Exercises

### Exercise 1 – type()

<DataCampExercise
  lang="python"
  hint={`Use type() to check the type of a variable at runtime.`}
  code={`# Task: Exercise 1 – type()
...
# Example structure: print(type(x))
...
x = "hello"
print(type(x))`}
  sct={`test_output_contains("<class 'int'>")
test_output_contains("<class 'str'>")
success_msg("type() works!")`}
  height={150}
/>

### Exercise 2 – isinstance()

<DataCampExercise
  lang="python"
  hint={`isinstance() checks if a variable is an instance of a given type.`}
  code={`# Task: Exercise 2 – isinstance()
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!

# Hint: 'for item in iterable:' iterates over each element in sequence.
# Step: Assign the correct value (a list) to 'values'.
values = ___  # replace ___ with the correct value
# Hint: print(value) displays output; print(a, b) prints multiple values.
# Step: Write a for loop -- 'for <variable> in <iterable>:'
for v in values:
    # Hint: isinstance() checks if a variable is an instance of a given type.
    # Step: Call print() with the right argument(s).
    # Example structure: print(v, "->", type(v).__name__)
    print(___)  # replace ___ with the correct expression

# ── Expected Output ──────────────────────────────────────────
# (Run your completed code to check the output)
# ──────────────────────────────────────────────────────────────`}
  solution={`values = [42, 3.14, "text", True]
for v in values:
    print(v, "->", type(v).__name__)`}
  sct={`test_output_contains("int")
test_output_contains("float")
test_output_contains("str")
success_msg("Dynamic typing inspection works!")`}
  height={160}
/>

### Exercise 3 – Dynamic Attribute Binding

<DataCampExercise
  lang="python"
  hint={`You can add attributes to objects dynamically in Python.`}
  code={`# Task: Exercise 3 – Dynamic Attribute Binding
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!
...
# Hint: Run after every change -- small steps are easier to debug!
# Step: Assign the correct value (a string) to 'o.name'.
o.name = ___  # replace ___ with the correct value
# Hint: Read error messages carefully -- they point to the exact line.
# Step: Assign the correct value (a number) to 'o.value'.
o.value = ___  # replace ___ with the correct value
# Step: Call print() with the right argument(s).
# Example structure: print(o.name, o.value)
print(___)  # replace ___ with the correct expression
...
print(o.name, o.value)`}
  sct={`test_output_contains("Dynamic 99")
success_msg("Dynamic attribute binding works!")`}
  height={170}
/>
