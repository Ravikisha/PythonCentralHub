...
### Exercise 1 – Lambda as Callable Object

<DataCampExercise
  lang="python"
  hint={`Lambda functions act like small anonymous callable objects.`}
  code={`# Task: Exercise 1 – Lambda as Callable Object
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!

# Hint: Lambda syntax: lambda args: expression  (single-line function)
# Step: Assign the correct value to 'square'.
square = ___  # replace ___ with the correct value
# Hint: print(value) displays output; print(a, b) prints multiple values.
# Step: Call print() with the right argument(s).
# Example structure: print(square(5))
...
print(square(5))`}
  sct={`test_output_contains("25")
success_msg("Lambda as callable works!")`}
  height={120}
/>
...
<DataCampExercise
  lang="python"
  hint={`Use type(name, bases, dict) to create a class dynamically.`}
  code={`# Task: Exercise 2 – type() to Create a Dynamic Class
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!

# Hint: Lambda syntax: lambda args: expression  (single-line function)
# Step: Assign the correct value to 'MyClass'.
MyClass = ___  # replace ___ with the correct value
# Hint: print(value) displays output; print(a, b) prints multiple values.
# Step: Assign the correct value to 'obj'.
obj = ___  # replace ___ with the correct value
# Hint: Use type(name, bases, dict) to create a class dynamically.
# Step: Call print() with the right argument(s).
# Example structure: print(obj.greet())
print(___)  # replace ___ with the correct expression

# ── Expected Output ──────────────────────────────────────────
# (Run your completed code to check the output)
# ──────────────────────────────────────────────────────────────`}
  solution={`MyClass = type("MyClass", (object,), {"greet": lambda self: "Hello!"})
obj = MyClass()
print(obj.greet())`}
  sct={`test_output_contains("Hello!")
success_msg("Dynamic class with type() works!")`}
  height={150}
/>

### Exercise 3 – Unnamed Nested Callable

<DataCampExercise
  lang="python"
  hint={`Use lambda in a list to simulate anonymous behavior.`}
  code={`# Task: Exercise 3 – Unnamed Nested Callable
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!

# Hint: 'for item in iterable:' iterates over each element in sequence.
# Step: Assign the correct value (a list) to 'actions'.
actions = ___  # replace ___ with the correct value
# Hint: Lambda syntax: lambda args: expression  (single-line function)
# Step: Assign the correct value (a number) to 'n'.
...
for action in actions:
    # Hint: Use lambda in a list to simulate anonymous behavior.
    # Step: Call print() with the right argument(s).
    # Example structure: print(action(n))
    print(___)  # replace ___ with the correct expression

# ── Expected Output ──────────────────────────────────────────
# (Run your completed code to check the output)
# ──────────────────────────────────────────────────────────────`}
  solution={`actions = [lambda x: x + 1, lambda x: x * 2, lambda x: x ** 2]
n = 3
for action in actions:
    print(action(n))`}
...
success_msg("Anonymous lambdas in list work!")`}
  height={170}
/>
