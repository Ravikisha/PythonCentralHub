...
## Try it: Python Tuple Exercises

### Exercise 1 – Create and Access a Tuple

<DataCampExercise
  lang="python"
  hint={`Create a tuple of 3 colors and access the second element.`}
  code={`# Task: Exercise 1 – Create and Access a Tuple
...
# Step: Assign the correct value (a tuple or function call) to 'colors'.
colors = ___  # replace ___ with the correct value
# Hint: Create a tuple of 3 colors and access the second element.
# Step: Call print() with the right argument(s).
# Example structure: print(colors[1])
...
print(colors[1])`}
  sct={`test_output_contains("green")
success_msg("Tuple creation and access works!")`}
  height={120}
/>

### Exercise 2 – Tuple Length and Iteration

<DataCampExercise
  lang="python"
  hint={`Use len() and a for loop to print each item.`}
  code={`# Task: Exercise 2 – Tuple Length and Iteration
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!

# Hint: 'for item in iterable:' iterates over each element in sequence.
# Step: Assign the correct value (a tuple or function call) to 'fruits'.
fruits = ___  # replace ___ with the correct value
# Hint: print(value) displays output; print(a, b) prints multiple values.
# Step: Call print() with the right argument(s).
# Example structure: print("Length:", len(fruits))
print(___)  # replace ___ with the correct expression
# Hint: Use len() and a for loop to print each item.
...
for fruit in fruits:
    print(fruit)`}
  sct={`test_output_contains("Length: 3")
test_output_contains("banana")
success_msg("Tuple iteration works!")`}
  height={170}
/>

### Exercise 3 – Tuple Immutability Check

<DataCampExercise
  lang="python"
  hint={`You can check that tuples are immutable by using type() and try/except.`}
  code={`# Task: Exercise 3 – Tuple Immutability Check
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!
...
# Hint: You can check that tuples are immutable by using type() and try/except.
# Step: Call print() with the right argument(s).
# Example structure: print("Type:", type(t))
print(___)  # replace ___ with the correct expression
# Hint: Run after every change -- small steps are easier to debug!
# Step: Call print() with the right argument(s).
# Example structure: print("Is tuple:", isinstance(t, tuple))
print(___)  # replace ___ with the correct expression
...
print("Is tuple:", isinstance(t, tuple))`}
  sct={`test_output_contains("Is tuple: True")
success_msg("Tuple type check works!")`}
  height={140}
/>
