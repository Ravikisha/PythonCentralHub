...
## Try it: Tuple Operations Exercises

### Exercise 1 – in Operator

<DataCampExercise
  lang="python"
  hint={`Use the in operator to check if an element exists in the tuple.`}
  code={`# Task: Exercise 1 – in Operator
...
success_msg("in operator works on tuples!")`}
  height={140}
/>

### Exercise 2 – Repeat a Tuple

<DataCampExercise
  lang="python"
  hint={`Use * to repeat a tuple.`}
  code={`# Task: Exercise 2 – Repeat a Tuple
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!
...
# ──────────────────────────────────────────────────────────────`}
  solution={`t = (1, 2)
print(t * 3)`}
  sct={`test_output_contains("1, 2, 1, 2, 1, 2")
success_msg("Tuple repetition works!")`}
  height={110}
/>

### Exercise 3 – min(), max(), sum()

<DataCampExercise
  lang="python"
  hint={`Use built-in min(), max(), and sum() on a numeric tuple.`}
  code={`# Task: Exercise 3 – min(), max(), sum()
# Follow the steps below and fill in the blanks.
# Use the 'Show Solution' button only after you have tried!
...
# Hint: Use built-in min(), max(), and sum() on a numeric tuple.
# Step: Call print() with the right argument(s).
# Example structure: print("Min:", min(nums))
print(___)  # replace ___ with the correct expression
# Hint: Run after every change -- small steps are easier to debug!
# Step: Call print() with the right argument(s).
# Example structure: print("Max:", max(nums))
print(___)  # replace ___ with the correct expression
...
print("Max:", max(nums))
print("Sum:", sum(nums))`}
  sct={`test_output_contains("Min: 1")
test_output_contains("Max: 9")
success_msg("min/max/sum work on tuples!")`}
  height={160}
/>
